...
{
    public class Decremento : Instruccion
    {
        private Instruccion op;
        int fila, columna;
        public Decremento(Instruccion op, int fila, int columna)
        {
            this.fila = fila;
        ...
        public object ejecutar(Contexto ctx, Stuff stuff)
        {
            if (op is Operacion)
            {
                ...
                if(op1.tipo == Operacion.Tipo.IDENTIFICADOR)
                {
                    ...
                    if (s == null)
                    {
                        stuff.error("Semántico", "'DECREMENTO', la variable '" + op1.value.ToString() + "' no existe.", op1.getLinea(), op1.getColumna(), ctx);
                        return null;
                    }
                    ...
                    stuff.error("Semántico", "'DECREMENTO', el tipo del operando es incompatible con esta instruccion. Encontrado: " + Operacion.getTipo(s.value) + ", Esperado: 'ENTERO', 'DOUBLE', 'CHAR'.", fila, columna, ctx);
                    return null;
                }
                else
                {
                    ...
                    stuff.error("Semántico", "'DECREMENTO', el tipo del operando es incompatible con esta instruccion. Encontrado: " + Operacion.getTipo(resOp) + ", Esperado: 'ENTERO', 'DOUBLE', 'CHAR'.", fila, columna, ctx);
                    return null;
                }
            }
            else if (op is AccesoArreglo)
            {
                ...
                if (resEx is Arreglo)
                {
                    ...
                    if (resIndice is BigInteger index)
                    {
                        ...
                        stuff.error("Semántico", "'DECREMENTO', el tipo del elemento indizado es incompatible con esta instruccion. Encontrado: " + Operacion.getTipo(arr.val.ElementAt((int)index)) + ", Esperado: 'ENTERO', 'DOUBLE', 'CHAR'.", fila, columna, ctx);
                        return null;
                    }
                        ...
                        else
                        {
                            ...
                            foreach (Instruccion i in m.listaInstrucciones)
                            {
                                ...
                                if (res is Return r)
                                {
                                    ...
                                    stuff.error("Semántico", "'DECREMENTO', el tipo del operando es incompatible con esta instruccion. Encontrado: " + Operacion.getTipo(r.expresionEvaluada) + ", Esperado: 'ENTERO', 'DOUBLE', 'CHAR'.", fila, columna, ctx);
                                    return null;
                                }
                            }
                            if (!m.esVoid)
            ...
            {
                stuff.error("Semántico", "'DECREMENTO', el tipo del operando es incompatible con esta instruccion. Encontrado: " + Operacion.getTipo(op) + ", Esperado: 'EXPRESION', 'LLAMADA METODO', 'VARIABLE', 'PROPIEDAD' , 'INDIZADOR'.", fila, columna, ctx);
            }
            return null;
        }
